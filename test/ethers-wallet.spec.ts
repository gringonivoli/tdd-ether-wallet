...
describe("EthersWallet Contract", function () {
    ...
    it('should send ethers to contract if not owner failing', async () => {
        const beforeBalance = await addr2.getBalance();
        await addr2.sendTransaction({ to: ethersWallet.address, value: ethers.utils.parseEther('1.0') });
        expect(await addr2.getBalance() < beforeBalance).to.equal(true);
    })
